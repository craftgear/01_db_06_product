...
export default function RepositoryListItem({ item }: Props) {
  return (
    <Link href={`/detail/?owner=${item.owner.login}&name=${item.name}`} target="_blank" rel="noopener noreferrer" className="no-underline"  >
      <div className="card rounded-2xl bg-base-100 shadow-sm ">
        <div className="card-body flex flex-row justify-start space-x-7">
