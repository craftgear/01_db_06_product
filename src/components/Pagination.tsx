...
export default function Pagination({ query, page, limit, total }: Props) {
  ...
  const totalPages =
    total > MAX_COUNT ? Math.ceil(MAX_COUNT / limit) : Math.ceil(total / limit);

  const prevPageLink = page > 1 ? buildQueryString(query, page - 1, limit) : "";
  const nextPageLink =
    totalPages > page ? buildQueryString(query, page + 1, limit) : "";
  ...
  return (
    <div className="w-full flex justify-center items-center mt-4 space-x-4 pb-20">
      {prevPageLink ? (
        ...
        <div className="btn btn-disabled join-item ">前のページ</div>
      )}
